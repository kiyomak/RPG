...
[作成クラスの概要]
- Livingクラスはすべての生物全般を表現するスーパークラス（抽象クラス）です
- HumanクラスはLivingクラスを継承し、人間全般を表現するスーパークラス（抽象クラス）です
- MonsterクラスはLivingクラスを継承し、モンスター全般を表現するスーパークラス（抽象クラス）です
- Brave, Fighter, Wizard の3つの具象クラスは Humanクラスを継承します。今回の課題では人間グループの構成メンバーとなります
- Slime, Oak, Dragon の3つの具象クラスは Monsterクラスを継承します。今回の課題ではモンスターグループの構成メンバーとなります
※　具象クラスとはインスタンスを作成できるクラスのことです。

[戦闘のシュミレーションの流れ]
1. Brave, Fighter, Wizard の3つのインスタンスからなる人間グループを準備します。それぞれの登場キャラクターは、名前、武器名、ヒットポイント（生命力）、攻撃力を持っています。
2. Slime、Oak、Dragon の3つのインスタンスからなるモンスターグループを準備します。それぞれの登場キャラクターは、名前、武器名、ヒットポイント（生命力）、攻撃力を持っています。
3. 人間グループとモンスターグループがグループ対戦します。
4. グループ対戦は、決着がつくまで以下の流れを繰り返します（繰り返し単位を回戦と呼ぶとします）
    4-1. 現在の人間グループから、サイコロを振って対戦に登場する人間インスタンスを1つ選びます
    4-2. 現在のモンスターグループから、サイコロを振って対戦に登場するモンスターインスタンスを1つ選びます
    4-3. 選ばれた人間インスタンスが、選ばれたモンスターインスタンスを攻撃します
    4-4. 攻撃されたモンスターインスタンスのヒットポイントが0以下になれば、そのモンスターインスタンスはモンスターグループから削除されます
    4-5. この時点でモンスターグループに誰もいなくなったら、人間グループの勝ちです
    4-6. 再度対戦に登場する人間インスタンスを1つ選び直します
    4-7. 再度対戦に登場するモンスターインスタンスを1つ選び直し選びます
    4-8. 今度は逆に 選ばれたモンスターインスタンスが、選ばれた人間インスタンスを攻撃します
    4-9. 攻撃された人間インスタンスのヒットポイントが0以下になれば、その人間は人間グループから削除されます
    4-10. この時点で人間グループに誰もいなくなったら、モンスターグループの勝ちです
...
public class Main {
	...
	public static void showGroupInfos(List<Human> humans, List<Monster> monsters) {
		...
		for (Human human : humans) {
			// .toString()メソッド（このオブジェクトを「テキストで表す」文字列を返す：文字を出力するもの（数値も、数字のテキストにして出力））
			//System.out,println(引数)の、（）に暗黙的につけられているもの。本来は、(引数.toString()）
			//明示しなくても.toString()は適用されるが、分かりやすいように明示した！今後も明示の方向で
...
/*
 メンターさん評価ポイント
 ・要件通りのコード・機能になっています。
・戦闘のシミュレーションの流れ通りに動作しています。
・攻撃後、HPがゼロ以下になっているキャラがいたら、メッセージを表示してListから取り除き、結果、Listが空になっていたらそのパーティーの負けになっています。
・インデントが揃ったきれいなコードが書けています。
・適宜コメントがあり、第三者に対して優しいコードになっています。
 
 ！！！第三者に分かりやすいコードを意識すること！！！
 */